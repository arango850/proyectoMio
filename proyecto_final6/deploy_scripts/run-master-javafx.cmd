...
:: run-master-javafx.cmd [MASTER_HOST] [CSV_PATH] [OUTPUT_PATH] [NUM_PARTITIONS] [WORKER_ENDPOINTS_COMMA_SEP]
if "%1"=="" (set MASTER_HOST=x104m01) else set MASTER_HOST=%1
if "%2"=="" (set CSV_PATH=datagrams4history.csv) else set CSV_PATH=%2
if "%3"=="" (set OUTPUT_PATH=master_output.csv) else set OUTPUT_PATH=%3
if "%4"=="" (set NUM_PARTS=30) else set NUM_PARTS=%4
if "%5"=="" (
...
) else set WORKER_ENDPOINTS=%5

:: Default JavaFX lib path (relative to script)
if "%JAVAFX_LIB%"=="" set "JAVAFX_LIB=%~dp0..\javafx-sdk-25.0.1\lib"

echo Starting Master on %MASTER_HOST% with %NUM_PARTS% partitions
echo Master endpoint: MasterService:tcp -h %MASTER_HOST% -p 20000
echo Workers: %WORKER_ENDPOINTS%

:: Run with JavaFX modules on module-path
java --module-path "%JAVAFX_LIB%" --add-modules javafx.controls,javafx.fxml -jar "..\master\target\master-0.1.0-SNAPSHOT-shaded.jar" "MasterService:tcp -h %MASTER_HOST% -p 20000" "%CSV_PATH%" "%OUTPUT_PATH%" %NUM_PARTS% "%WORKER_ENDPOINTS%"
endlocal